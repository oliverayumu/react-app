pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("oliverayumu/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull oliverayumu/react-app:${env.BUILD_NUMBER}"
                        try {
                ...
                input 'Does the staging environment look OK? Did You get 200 response?'
                 milestone(1)
                    script {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d oliverayumu/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
    }
